...
class InferenceEngine:
    ...
    def forward_chaining(self) -> Dict[str, float]:
        ...
        while changed and iteration < 100:  # Limit iterasi untuk menghindari infinite loop
            changed = False
            iteration += 1
                ...
                if self.can_fire_rule(rule):
                    ...
                    if rule_cf > 0:
                        ...
                        if conclusion in self.facts:
                            ...
                            trace_msg = f"Rule {rule['id']}: {' + '.join(rule['if'])} → {conclusion}"
                            trace_msg += f" (CF: {rule_cf:.3f}, gabungan: {old_cf:.3f} + {rule_cf:.3f} = {self.facts[conclusion]:.3f})"
                            print(f"  Aturan paralel {rule['id']}: {conclusion} CF {old_cf:.3f} → {self.facts[conclusion]:.3f}")
                            
                        else:
                            # Kesimpulan baru (aturan sekuensial)
                            self.facts[conclusion] = rule_cf
                            trace_msg = f"Rule {rule['id']}: {' + '.join(rule['if'])} → {conclusion} (CF: {rule_cf:.3f})"
                            print(f"  Rule {rule['id']}: {conclusion} (CF: {rule_cf:.3f})")
    ...
    def infer(self, initial_facts: Dict[str, float]) -> Dict[str, Any]:
        ...
        # Get most likely conclusion
        most_likely = self.get_most_likely_conclusion()
        
        # Format results for web API
        formatted_results = []
        for conclusion, cf in conclusions.items():
            if conclusion not in initial_facts:  # Exclude input symptoms from results
        ...
        if formatted_results:  # Get the highest CF result from our formatted results
        ...
        return {
            ...
            'processing_info': {
                ...
                'facts_derived': len([fact for fact in results.keys() if fact not in initial_facts])
            }
        }
    ...
    def print_results(self, threshold: float = 0.0):
        ...
        for fact, cf in sorted_facts:
            if cf >= threshold:
                confidence_level = "Sangat Tinggi" if cf >= 0.8 else "Tinggi" if cf >= 0.6 else "Sedang" if cf >= 0.4 else "Rendah"
