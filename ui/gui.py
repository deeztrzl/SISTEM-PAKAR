...
class ExpertSystemGUI:
    ...
    def create_widgets(self):
        ...
        for i, symptom in enumerate(self.available_symptoms):
            ...
            tk.Label(cf_frame, text="CF:", font=("Arial", 8), bg="#f0f0f0").pack(
                side="left"
            )
        ...
        trace_btn = tk.Button(
            right_frame,
            text="Tampilkan Jejak Inferensi",
    ...
    def display_results(self, input_symptoms, results):
        ...
        if results:
            ...
            for fact, cf in sorted_results:
                if cf >= 0.1:  # Hanya tampilkan yang signifikan
                    ...
                    self.results_text.insert(
                        tk.END, f"  CF: {cf:.3f} ({confidence_level})\\n\\n"
                    )
            ...
            if most_likely:
                ...
                self.results_text.insert(tk.END, f"• {display_name}\\n")
                self.results_text.insert(
                    tk.END, f"  Tingkat Keyakinan: {highest_cf:.3f}\\n"
                )
                self.results_text.insert(
                    tk.END, f"  Level: {self.get_confidence_level(highest_cf)}\\n"
                )
        else:
            self.results_text.insert(
                tk.END, "Tidak ada kesimpulan yang dapat ditarik.\\n"
            )
    ...
    def show_inference_trace(self):
        ...
        if not trace:
            messagebox.showinfo(
                "Info", "Belum ada jejak inferensi. Jalankan diagnosa terlebih dahulu."
            )
            return
